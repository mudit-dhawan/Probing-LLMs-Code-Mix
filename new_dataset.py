...
class LIDdataset():
    ...
    def __getitem__(self, idx):
        ...
        ids, masking_labels = mask_sentence(ids, langtags, self.tokenizer)
        ...
        ids = config.TOKENIZER.build_inputs_with_special_tokens(ids)
...
import random
def mask_sentence(token_ids, lang_ids, tokenizer, style='bert'):
    """
    ...
    # find possible lang_ids
    possible_lang_ids = set(lang_ids)

    # store weights to be decided based on the occurence of a particular lang_id
    wt_lang_ids = {}

    for lang_id in possible_lang_ids:
        # more weight given to less occuring lang_id
        wt_lang_ids[lang_id] = 1 - (len([1 for x in lang_ids if x==lang_id]) / len(lang_ids))

    # define the max and min possible masking probability for a token 
    min_prob_mask = 0.15
    max_prob_mask = 0.3 

    # store the masking probability for each lang_id
    prob_mask_lang_ids = {}

    for lang_id in possible_lang_ids:
        # probabilty of masking can be between the limits
        prob_mask_lang_ids[lang_id] = max(max_prob_mask*(wt_lang_ids[lang_id]), min_prob_mask)
    
    # print(prob_mask_lang_ids)
    
    for i, token_id in enumerate(token_ids):
        prob = random.random()

        # mask token with new probability based on lang id
        prob_mask = prob_mask_lang_ids[lang_ids[i]]
